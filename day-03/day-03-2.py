...
def count_steps(wire_coords, intersection_coords):
  count_dict = {'{},{}'.format(str(x), str(
      y)): 0 for (x, y) in intersection_coords}
  step = 0
  for (x, y) in wire_coords:
    step += 1
    dict_key = '{},{}'.format(str(x), str(y))
    if dict_key in count_dict and count_dict.get(dict_key) == 0:
      count_dict[dict_key] = step
  return count_dict


wire_one_coords_list = get_coords(wire_one)
wire_one_coords = set(wire_one_coords_list)
wire_one_coords.remove((0, 0))

wire_two_coords_list = get_coords(wire_two)
wire_two_coords = set(wire_two_coords_list)
wire_two_coords.remove((0, 0))

intersection = wire_one_coords.intersection(wire_two_coords)
